...
function isNeedAddInQuery(mappings, columnObjName) {
    return !mappings[columnObjName].IsAutoIncrement;
}
...
TableMap.prototype.Insert = function (entity) {
    ...
    for (var prop in cols) {
        if (!self.hasOwnProperty(prop)) continue;
        if (isNeedAddInQuery(self, prop)) {
...
TableMap.prototype.columnMap = function (objProperty, tableProperty, config) {
    ...
    if (config) {
        var isAutoIncrement = config.isAutoIncrement || false;
    }
